...
const thoughtArticles = [
  ...
  },
  {
    ...
    label: "做了产品，不等于懂产品",
    text: "功能交付需要同时覆盖产品目标、业务链路、架构选择、底层机制和替代方案。",
    lead: "项目驱动学习：用真实问题锁定知识范围，再把产品背景、源码机制、架构取舍和复盘记录沉淀下来。",
    sections: [
      {
        title: "认知差：能做不等于能讲清",
        paragraphs: [
          "常见风险：需求进入开发后只完成页面和接口，却缺少产品目标、业务闭环和机制边界的沉淀。",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "开发过程中追踪技术上下文：代码职责、方案取舍、底层机制和替代成本。追问清楚之后，交付才会变成经验。"
        ]
      },
      {
        title: "项目倒逼学习",
...
function VibePrinciples() {
  ...
  const learningLoop = [
    ["补上下文", "明确产品目标、用户痛点、上下游链路和行业主流解法。"],
    ["边做边溯源", "追踪代码职责、架构取舍、运行机制和替代方案。"],
    ["复盘成文章", "沉淀方案亮点、验证结果、风险边界和下一步计划。"],
    ["知识库反哺", "把项目经验转化为可复用素材，继续提升产品设计和工程判断。"]
  ];

  const threeEPrinciples = [
    ["Easy to See", "核心价值一屏可见。信息层级清晰，项目目标、使用场景和结果证据直接呈现。"],
    ["Easy to Use", "上手路径足够短。核心动作清楚，交互反馈及时，用户能顺利完成第一次使用。"],
    ["Easy to Grace", "体验稳定顺滑。状态切换自然，异常处理清楚，细节让用户愿意继续使用。"]
  ];

  const valueEnds = [
    ["前端：发现力", "定位高价值场景，完成市场调研、痛点识别和赛道选择，把工程投入放到正确问题上。"],
    ["后端：放大器", "完成产品包装、价值传递、宣发引流和商业化落地，让项目被看见、被使用、被传播。"]
  ];
  ...
  return (
    <section id="vibe" className="section vibe-section">
      <div className="section-head">
        ...
        <p>Agent 工程交付可运行的产品、可验证的过程证据和可复用的知识结构。</p>
      </div>
      ...
      <div className="context-essay">
        <div>
          ...
          <p>
            一个好 APP 会把身份、权限、状态、任务进度和历史行为组织成连续上下文。用户是谁、能做什么、刚才完成了什么、当前任务到哪一步，都会影响下一次交互。
          </p>
          <p>
      ...
      <div className="three-e-panel">
        <div>
          ...
          <p>
            清晰价值、短路径和稳定体验构成产品的基础标准。访问者能快速判断项目做什么、如何开始、是否值得继续使用。
          </p>
        </div>
      ...
      <div className="value-ends-panel">
        <div>
          ...
          <h3>两端价值理论</h3>
          <p>
            优秀项目同时需要发现问题和放大结果。前端负责找到真实需求与可行动场景，后端负责把价值包装、分发并转化为持续使用。
          </p>
        </div>
      ...
      <div className="learning-loop">
        <div>
          ...
          <h3>边做产品，边构建知识库</h3>
          <p>
            每个项目沉淀需求、方案、验证、发布记录和复盘结论，形成持续升级的工程知识库。
          </p>
        </div>
      ...
      <div className="quote-box dark">需求拆解、技术路线、验证截图、发布记录和复盘结论共同构成项目证据链。</div>
    </section>
  );
}
    ...
    <main className="paper-detail">
      ...
      <section className="detail-section">
        ...
        <div className="reading-notes">
          <p>
            快速阅读路径：摘要、方法框架、动机图、实验主表和消融表。读者可以先锁定研究问题，再判断方法收益。
          </p>
          <p>
            复现阅读路径：输入输出定义、核心模块训练目标、消融设置和失败案例。图表用于识别关键模块、辅助模块和风险边界。
          </p>
          <p>
            论文价值判断：研究痛点是否清晰，方法结构是否可解释，实验证据是否覆盖主结果、消融和可视化。
          </p>
        </div>
      </section>
      <section className="detail-section">
